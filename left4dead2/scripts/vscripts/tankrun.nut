...
{
	SessionState.RemoveInfectedFailsafe = true;
...
function OnGameEvent_finale_start( params )
{
	...
	if ( NetProps.GetPropInt( finale, "m_type" ) == 4 )
		return;
...
function Update()
{
	if ( SessionState.LeftSafeAreaThink )
		LeftSafeAreaThink();
	if ( SessionState.RemoveInfectedFailsafe )
		RemoveInfectedFailsafe();
